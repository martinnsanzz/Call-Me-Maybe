...
<div align="center">
  <img src="https://img.shields.io/badge/SCORE-110-%235CB338?style=for-the-badge&logo=42&logoColor=white"/>
  <img src="https://img.shields.io/badge/Bonus-40%25-orange?style=for-the-badge&logo=starship&logoColor=white"/>
...
`Call-Me-Maybe` focuses on building a function calling tool that translates natural language
prompts into structured, schema-compliant function calls, without ever letting the language
model "hallucinate" free-form text. I utilize an llm_sdk provided by the subject using the
model *'Qwen/Qwen3-0.6B'* from HuggingFace websiteThe. Given a specific user prompt
*(e.g., "What is the sum of 2 and 3?")*, the system must not answer the question directly, but
instead output the appropriate function name and extract the required arguments.

**📊 The Challenge:**
The core challenge is that the model (a small, ~0.6B-parameter LLM accessed through
the `llm_sdk` package) is not reliable enough to produce valid JSON. 

**💡 The approach:**
Instead of prompting and hoping, this project drives the model's generation **token by token**, 
masking its output logits at every step so that only tokens consistent with a valid function
name, a valid argument type, and the content of the original prompt (This is called 
`constrained decoding`). The output is always syntactically valid and always grounded in 
something the user actually wrote — accuracy is enforced structurally, not statistically.
...
The program must generate a single output file named `function_calling_results.json`. Each object within this array must strictly contain the `prompt`, the function `name`, and the `parameters`. The final solution must achieve over 90% accuracy for function selection and prompt selection. Additionally it must process all test prompts in under 5 minutes (This is also based on the computer architecture so it might differ in performance).
...
This project contains a Makefile with the following rules:
- `make install`: Create venv and install dependencies with uv
- `make run `: Run the program
- `make test `: Run the program with test .json file path
- `make debug`: Run with debugger (pdb by default)
- `make visual`: Run the program showing each step
...
The project uses `uv` for dependency management. In order to run the project, first you need to
install all the dependencies in a `venv` using `Python3`. Make sure you have `venv` module
installed in your computer.

```bash
make install # install all dependencies and run the script

uv sync  # alternatively you can also use this
```

**Note:** If you dont have `uv` installed used `make install`.
...
Using `uv run python -m src` (or `make run`) you will launch the program using the default 
arguments. Alternatively you can use `make visual` or `make test` based on how or what you want
to display.

If you want to use your own files, use the following flags

| FLAGS | DEFINITION |
|:-----:|:--------------:|
| --functions_definition | path where function definition in json is stored |
--input |  path where input file in json is stored |
 --output |  path to the output file in json |

 You can also use two other flags:
 - **--visual_mode**: To show the visual progress of the LLM.
 - **--test**: To use a test file inside `data/test`.
...
possibility of each token and select only one that would be still consider a valid json if 
choosen and that would be a lot of steps even before attempting to extract the function names 
and the parameters.
